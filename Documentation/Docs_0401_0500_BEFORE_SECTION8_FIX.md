...
## 1️⃣ Project 0401: Introduction to LED Patterns
...
## 1️⃣ Project 0402: Blinking LED Patterns
...
## 1️⃣ Project 0403: Manual LED Patterns Control
...
**A. Initialization Phase**
1.  **Configure Pins**:
    *   From **Smart IO**, drag `pico_analog_setup` and configure GP26 as ADC input.
    *   From **Smart IO**, drag `pico_pwm_setup` and configure GP15 as PWM output.

**B. Main Loop Phase**
2.  **Read Light Level**:
    *   From **Smart IO**, **Read** analog value from GP26 → store in `lightLevel`.
3.  **Map to Frequency**:
    *   From **Math**, **Map** `lightLevel` from range [0-65535] to [200-2000] → store in `frequency`.
    *   Note: Lower ADC values (darkness) → lower frequencies, higher ADC values (brightness) → higher frequencies.
4.  **Generate Tone**:
    *   From **Smart IO**, **Set** PWM frequency on GP15 to `frequency` Hz.
    *   From **Smart IO**, **Set** PWM duty cycle to 50% (square wave).
5.  **Sampling Delay**:
...
1.  **Setup PWM**:
    ...
    *   From **Variables**, create list `notes = [262, 330, 392]`.

**B. Main Loop Phase**
2.  **Play Arpeggio**:
    *   From **Loops**, **For** each `currentNote` in `notes`:
        *   From **Smart IO**, **Set** PWM frequency to `currentNote`.
        *   From **Smart IO**, **Set** PWM duty cycle to 50%.
        *   from **Smart IO**, **Wait** 0.5 seconds.
3.  **Silence Between Cycles**:
    *   From **Smart IO**, **Set** PWM duty cycle to 0%.
    *   from **Smart IO**, **Wait** 0.5 seconds.
