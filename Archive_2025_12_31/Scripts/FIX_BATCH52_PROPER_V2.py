...
def fix_batch_52():
    # Fix the delimiters to use triple double quotes consistently
...
**A. Initialization Phase**
1.  **Init Hardware**: Standard setup.
...
2.  **Control Player**:
    ...
    *   If GP14: px += 2.
3.  **Move Enemy**:
    *   ey += 2. If ey > 64: reset ey=0.
4.  **Collision Check**:
    *   If dist between (px, 60) and (ex, ey) < 3: Break game.
5.  **Render**:
...
**B. Main Loop Phase**
2.  **Activity Sensor**:
    *   If button pressed: reset `idle` timer.
3.  **Logic**:
    *   If current_time - `idle` > 10s: Run Bouncing Text.
    *   Else: Display "Active".

### 9. Execution Flow
Monitors time since last input to trigger screensaver.
...
**B. Main Loop Phase**
2.  **Animate Mouth**:
    *   If `frame` = 0: Draw ellipse with "pie slice" cut.
...
4.  **Iterate**:
    *   `frame` = 1 - `frame`.
5.  **Show**:
    *   `oled_clear`, draw shape, `oled_show`.
...
"""
    ...
    # Correcting the replacement logic to encompass the whole of Batch 52 correctly
    ...
    batch52_end = content.find('# ', batch52_start + 1)

    if batch52_start != -1:
        before = content[:batch52_start]
        after = content[batch52_end:] if batch52_end != -1 else ""
        new_doc = before + batch_52_content + after
        with open(r'd:\MFF\Pico\Documentation\Docs_0501_0600.md', 'w', encoding='utf-8') as f:
            f.write(new_doc)
        print("Batch 52 successfully fixed in Docs_0501_0600.md")
    else:
        print("Could not locate Batch 52 start string.")
